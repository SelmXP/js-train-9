...
function findIndexByProperty(arr, name) {
  ...
  return foundIndex; // Повертаємо знайдений індекс або -1, Якщо жоден елемент не задовольняє умову
}
...
function findLastEvenNumber(arr) {
  ...
  const foundLast = arr.findLast((element) => element % 2 === 0);

  return foundLast !== undefined ? foundLast : "Парних чисел не знайдено";
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const cut_arr = arr.slice(startIdx);
  return cut_arr.join(" ");
}
...
function getArrayKeys(arr) {
  ...
  return Array.from(arr.keys(arr));
}
...
function getPositiveArrayValues(arr) {
  ...
  // const arrPositive = Array.from(arr.values());
  // return arrPositive.filter((element) => element > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  arr.splice(startIdx);
  const arrDouble = arr.map((element) => (element *= 2));
  return arrDouble;
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const arrFlatten = arr.flat();
  const arrSquared = arrFlatten.map((element) => element ** 2);
  const arrFiltred = arrSquared.filter((element) => element > threshold);
  return arrFiltred;
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const arrSliced = arr.slice(-n);
  const sum = arrSliced.reduceRight((accum, current) => accum + current);
  arrSliced.forEach((element) => console.log(element));
  return sum;
}
...
function wordLengths(arr) {
  ...
  if (!Array.isArray(arr)) return "Вхідний параметр має бути масивом";
  return Array.from(arr.map((word) => word.length));
}
...
function flattenArray(arr) {
  ...
  if (!Array.isArray(arr)) return "Вхідний параметр має бути масивом";
  const arrFlattenInfinity = arr.flat(Infinity);
  return arrFlattenInfinity;
}
...
function expandArrayByN(arr, n) {
  ...
  const arrExpand = arr.flatMap((value) => [...newArray.fill(value)]);

  // 2й варіант
  // const arrExpand = arr.flatMap((value) => [...Array(n).fill(value)]);
...
function findLongestWord(arr) {
  ...
  return arr.reduce((longWord, currentWord) => {
...
function findDuplicateElements(arr) {
  ...
  const arrDublicate = arr.filter((element, index) => {
...
function capitalizeWords(arr) {
  ...
  if (!Array.isArray(arr)) return [];
  const newArray = arr.map((word) => word[0].toUpperCase() + word.slice(1));
  return newArray;
}
